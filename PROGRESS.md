...
| **阶段六** | 性能优化+上下文+成本+人机+多智能体 | ✅ 完成 (7/8) | 五方案+四模块全交付 |
...
| V6.0 | 阶段六：5方案性能优化 + 上下文管理 + 成本追踪 + 人机协同 + 多智能体 | ✅ (7/8) |
...
## 全项目系统性审计报告 (2026-07-08 晚)

**审计范围**：`backend/` 下全部 50+ Python 文件 | **维度**：线程安全 / 内存泄漏 / 错误处理 / 架构风险

### 问题清单（12 项按严重程度分级）

| # | 严重 | 模块 | 问题 | 文件:行号 |
|---|:--:|------|------|-----------|
| 1 | 🔴 | graph | executor_node 仍用 ThreadPoolExecutor，web 路径首次请求可能触发模型双重加载 | `graph.py:192` |
| 2 | 🔴 | hybrid_search | `_get_lambda_mart()` 懒加载无锁 — CrossEncoder(~1.5GB) | `hybrid_search.py:210` |
| 3 | 🔴 | embedder | `get_embedding_model()` 懒加载无锁 — BGE(~400MB) | `embedder.py:20` |
| 4 | 🔴 | vector_store | `_get_chroma()` 懒加载无锁 | `vector_store.py:19` |
| 5 | 🔴 | hybrid_search | `_get_bm25_index()` 懒加载无锁 — 全量文档索引 | `hybrid_search.py:74` |
| 6 | 🔴 | api/agent | `run_agent_stream` 同步迭代阻塞 FastAPI 事件循环 | `api/agent.py:66` |
| 7 | 🟠 | api/rag | `_session_store` 与 `redis_client.SessionStore` 双写冲突 | `api/rag.py:35` vs `redis_client.py:133` |
| 8 | 🟠 | api/rag | DB 写入在 `_session_lock` 锁内执行 | `api/rag.py:48-55` |
| 9 | 🟠 | db | SQLite `check_same_thread=False` 并发写入可能 "database is locked" | `database.py:20` |
| 10 | 🟡 | monitor | RateLimiter `_memory_store` 过期 IP 不清理 | `monitor.py:59` |
| 11 | 🟡 | evaluator | `_eval_report_cache` 无 TTL | `evaluator.py:518` |
| 12 | 🟡 | api/rag | SSE 异常 `logger.error()` 缺 `exc_info=True` | `api/rag.py:288` |

### 建议修复方案

**P0-1~5（线程安全）**：4 个全局单例加双重检查锁（DCL），或暂关 executor_node 并行
**P0-6（事件循环阻塞）**：`run_agent_stream` 放入 `loop.run_in_executor()` 线程池
**P1-7~9（架构）**：session 统一到 SessionStore；DB 写入移出锁；SQLite 加 `?timeout=30`
**P2-10~12（优化）**：加过期清理、`exc_info=True`、缓存 TTL
