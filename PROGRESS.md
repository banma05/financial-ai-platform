...
> 📅 最后更新：2026-07-18
> 🎯 当前版本：**V8.3 🔄 收尾中（9/12 完成 + 向量库重建 + HNSW 自愈）**
> 🏗️ 状态：剩 Docker 验证 / SSE 浏览器验证 / easy 题分析 / 延迟优化重开

---

## V8.3 进展 (2026-07-17 ~ 07-18)

### 检索评测基线（07-18 重建库后）

| 指标 | V8.2 | V8.3 当前 | 目标 | 达标? |
|------|:--:|:--:|:--:|:--:|
| 语义召回 SEM-R@5 | 89.3% | **96.0%** | 90%+ | ✅ |
| 语义平均相似度 | — | 0.636 | — | — |
| 关键词 KW-R@5 | N/A | 73.2% (15题标注) | 可用 | ✅ |
| ChromaDB chunks | 3,042 | **5,097** (14文档) | 5,000+ | ✅ |
| 平均检索耗时(全量) | 26.0s | 31.9s | ≤8s | ❌ 重开 |

### 第一轮（07-17 下午）：V8.3 计划 9/12 项

- ✅ 1.1 数据库补 2022 年（实际 2021-2026 全覆盖，20家×55指标）
- ✅ 1.2 RAG 文档库扩充（+招股书×3 +研报，10→14 文档）
- ✅ 2.1-2.4 评测清理（R14/R15 边界分离、full_eval 编码、Agent 报告持久化、15题关键词全标注）
- ✅ 3.1 现金流准确率修复 + 3.3 CrossEncoder 熔断器
- ⚠️ 3.2 延迟优化（<20字短路方案 07-18 已回退，见下）

### 第二轮（07-17 深夜 ~ 07-18）：向量库根因修复

| 事项 | 说明 |
|------|------|
| 🔴 HNSW 损坏彻底解决 | 成因：大库 compaction 需数分钟，批量导入脚本退出打断 → 残缺 segment（有 pickle 缺 bin 文件）。双层防御：①启动自愈 `_heal_corrupt_segments()`（检测残缺→删除→WAL backfill 重建，官方推荐，零数据丢失）②退出确认 `wait_for_compaction(300s)` 接入 rebuild/import 脚本。真实链路端到端验证通过 |
| 🔴 切分 bug 修复 | <100字页面过滤（19a3416）误杀年报纯表格页——仅茅台2024就丢 9 页核心财务报表。决策：整个去掉过滤。chunks 4187→5097，检索命中此前丢失的报表页 |
| 🔴 路由回退 | e676817 的"<20字走simple"降级数值查询精度且带 7 个测试失败入库。回退后 test_hybrid_search 35/35 恢复全绿 |
| 🟡 残留清理 | 删除旧 chroma_db(75M)、_chunk_qa、_cq2、backend/data 误建目录 |

### V8.3 剩余项

| # | 事项 | 状态 |
|:--:|------|:--:|
| 1 | **修评分负号缺陷**：`bench_agent.py:218` 正则 `\d+\.?\d*` 丢负号，报告"-710.68亿"提取成 710.68 → 200% 差必 mismatch。失分全是负数指标（C01 投资/筹资CF、D01 财务费用）。修法：正则加 `-?` + 负值指标 abs() 候选 + 修 C01/C02 required_numbers 复制错误。预期现金流→100%、easy→100%、总准确率 97%→99%+ | ⬜ 根因已实锤 |
| 2 | RAG 延迟优化重开（CrossEncoder 复用+超时保护，31.9s→≤8s） | ⬜ |
| 3 | Docker 构建验证 (3.4) | ⬜ |
| 4 | 前端 SSE 浏览器验证 (3.5) | ⬜ |

> 07-18 凌晨全量评测快照：Agent 97.0%/97.6%/97.3%/11.6s | RAG SEM-R@5 96.0%、KW-R@5 73.2% | SQL 覆盖 100%、准确 95.8% | 后端 385 + 前端 19 全绿 | 成本 ¥0.56/15题
> easy 91.1% 反直觉已解谜：easy 组恰含 C01/D01 两道负数指标题，与现金流 88.9% 同根因，非拆解问题。0c19150 的"单位适配"修复方向错误、无效。
...
| # | 事项 | 状态 |
|:--:|------|:--:|
| 1 | ~~RAG 幻觉率评测~~ | ✅ **15/15来源引用, 100%合格, 平均5.0来源/题, 8.9s延迟** |
| 2 | Docker 一键部署验证 | ⬜ 未验证 |
| 3 | 前端 E2E 测试 | ⬜ 未实现 |
| 4 | chart dual_axis 修复 | ✅ **数据为空时降级柱状图** |
