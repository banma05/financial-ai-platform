...
> 📅 最后更新：2026-07-19 傍晚
> 🎯 当前版本：**V8.4 智能分析引擎（5提交）+ V8.3 全部完成（14/14）**
> 🏗️ 状态：数据准确性源头修复 + 智能图表 + 动态规划 + 专业研报，核心体验闭环
...
| # | 问题 | 方向 |
|:--:|------|------|
| 1 | PE/PB 仍不可用（SQL 无股价） | Planner 编排 mcp_stock_price |
| 2 | 前端仍有硬编码年份（如"2024年"） | 改为动态年份 |
| 3 | RAG 评测基线待重跑（Faithfulness/Answer Relevancy/Context Recall） | 跑 pro judge |
| 4 | 无网络环境下 AKShare 工具超时 | Mock 兜底策略优化 |
