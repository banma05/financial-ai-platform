...
| 任务类型 | 使用模型 | 原因 |
|----------|----------|------|
| 文档摘要 / 信息抽取 | `deepseek-v4-flash` | 快且便宜，适合简单任务 |
| 关键词提取 / 文本分类 | `deepseek-v4-flash` | 无需深度推理 |
| 财务指标分析 / 趋势判断 | `deepseek-v4-pro` | 深度推理能力 |
| 跨文档对比分析 / 异常检测 | `deepseek-v4-pro` | 复杂多步逻辑 |
| 文本向量化 | 本地 BGE 模型 | 免费，无需 API 调用 |

> 通过任务复杂度自动判断，在成本和效果之间取得平衡。
