...
class ToolRegistry:
    ...
    def _inject_dependency_data(
        ...
        例如：calculate 任务需要 net_profit 和 revenue，
        从 data_query 任务的结果中提取这些值。

        V2.5 MVP 实现简单注入策略：
        - 如果任务有 depends_on，从对应结果中提取所有数值字段
        - V3.0 增强为智能字段匹配（LLM 辅助）
        """
        ...
        for dep_id in task.depends_on:
            ...
            # 如果前置任务返回了结构化 data，提取其中的数值
            dep_data = dep_result.data
            if isinstance(dep_data, dict):
                extracted = dep_data.get("data", dep_data)
                if isinstance(extracted, dict):
                    # 将前置任务提取的数据合并到当前任务参数
                    for k, v in extracted.items():
                        if isinstance(v, (int, float)) and k not in params:
                            params[k] = v
