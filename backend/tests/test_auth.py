...
def _create_test_app(api_key: str = "test-secret-key", rate_limit_enabled: bool = True):
    ...
    返回: (app, TestClient, effective_key)
      - effective_key: 实际生效的 API Key（config.API_KEY 或自动生成的开发密钥）
    """
    import config
    from middleware import auth as auth_module

    # 重置开发密钥（避免测试间污染）
    auth_module._dev_api_key = ""
    ...
    # V8.1: 获取实际生效的 Key（配置值优先，否则自动生成的开发密钥）
    effective_key = auth_module._get_effective_key()

    return app, TestClient(app), effective_key
...
class TestAPIKeyAuth:
    ...
    def test_无APIKey配置_自动生成开发密钥(self):
        """V8.1: API_KEY 为空时自动生成开发密钥，无 Header 请求应 401"""
        _, client, effective_key = _create_test_app(api_key="")
        # 应自动生成了开发密钥
        assert effective_key.startswith("dev-"), f"预期 dev- 前缀，实际: {effective_key}"
        # 无 Header → 401（不再零鉴权对外开放）
        resp = client.get("/api/v1/rag/documents")
        assert resp.status_code == 401, f"预期 401，实际 {resp.status_code}"
        # 带正确开发密钥 → 200
        resp2 = client.get("/api/v1/rag/documents", headers={"X-API-Key": effective_key})
        assert resp2.status_code == 200

    def test_有APIKey_无Header_返回401(self):
        """配置了 API_KEY 但不带 Header → 401"""
        _, client, effective_key = _create_test_app(api_key="my-secret")
        assert effective_key == "my-secret"
        resp = client.get("/api/v1/rag/documents")
    ...
    def test_健康检查_不鉴权(self):
        """/health 端点始终跳过鉴权"""
        _, client, _ = _create_test_app(api_key="my-secret")
...
class TestRateLimit:
    ...
    def test_限流关闭_不拦截(self):
        ...
        _, client, dev_key = _create_test_app(api_key="", rate_limit_enabled=False)
        headers = {"X-API-Key": dev_key}
        for _ in range(10):
            resp = client.get("/api/v1/rag/documents", headers=headers)
            assert resp.status_code == 200

    def test_超限后返回429(self):
        """超过限制后返回 429 Too Many Requests"""
        _, client, dev_key = _create_test_app(api_key="", rate_limit_enabled=True)
        headers = {"X-API-Key": dev_key}
    ...
    def test_不同IP_独立计数(self):
        ...
        # 用一个 IP 发 10 个请求触发限流
        for _ in range(10):
            client.get("/api/v1/rag/documents", headers=headers)

        # 换一个 IP 模拟
        resp = client.get(
            "/api/v1/rag/documents",
            headers={"X-API-Key": dev_key, "X-Forwarded-For": "10.0.0.2"},
        )
        assert resp.status_code == 200
...
class TestAuthEdgeCases:
    ...
    def test_空APIKey_Header传空字符串_返回401(self):
        """V8.1: API_KEY 为空时已自动生成开发密钥，空字符串 Header → 401"""
        _, client, effective_key = _create_test_app(api_key="")
        assert effective_key.startswith("dev-")
