...
你必须逐个提取以下具体指标，每个指标作为 JSON 的一个键：
...
## 核心规则（违反即为错误）
1. **只输出一个扁平 JSON 对象**，每个键是具体指标名，值是该指标的数值或 null
2. **禁止使用分类名作为键**：不要输出 "利润表"/"资产负债表"/"现金流量表" 等分类名，必须拆成具体指标
3. 数值必须是纯数字，严禁带单位（"亿元""万元""%""元"全部去掉）
4. 金额类统一转为「亿元」单位（万元÷10000，元÷100000000）
5. 百分比类直接输出数值：92.38 而非 "92.38%" 或 "0.9238"
6. 找不到的指标输出 null

## ❌ 错误示例（千万不要这样输出）
```json
{{"利润表": {{"营业收入": 1709.90}}, "资产负债表": {{"总资产": 3425.12}}}}
```

## ✅ 正确示例
```json
{{"营业收入": 1709.90, "营业成本": 137.89, "净利润": 893.35, "总资产": 3425.12, "总负债": 456.78, "净资产": 2968.34}}
```

## 文档片段
{context}

## 请输出（只输出一行扁平 JSON，键是指标名，值是数字或 null，不要代码块标记）"""
